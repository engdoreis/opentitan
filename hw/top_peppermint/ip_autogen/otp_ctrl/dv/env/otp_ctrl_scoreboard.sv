...
class otp_ctrl_scoreboard #(type CFG_T = otp_ctrl_env_cfg)
      ...
      @(posedge cfg.otp_ctrl_vif.pwr_otp_done_o || cfg.under_reset ||
        ...
        if (!cfg.under_reset && !cfg.otp_ctrl_vif.alert_reqs && cfg.en_scb) begin
          ...
          exp_hw_cfg0_data = cfg.otp_ctrl_vif.under_error_states() ?
                             top_peppermint_rnd_cnst_pkg::RndCnstOtpCtrlPartInvDefault[HwCfg0Offset*8 +: HwCfg0Size*8] :
          ...
          exp_hw_cfg1_data = cfg.otp_ctrl_vif.under_error_states() ?
                             top_peppermint_rnd_cnst_pkg::RndCnstOtpCtrlPartInvDefault[HwCfg1Offset*8 +: HwCfg1Size*8] :
          ...
          if (!cfg.otp_ctrl_vif.under_error_states()) begin
            ...
            if (cfg.otp_ctrl_vif.lc_seed_hw_rd_en_i == lc_ctrl_pkg::On) begin
              ...
              exp_creator_root_key.share0 =
                  top_peppermint_rnd_cnst_pkg::RndCnstOtpCtrlPartInvDefault[CreatorRootKeyShare0Offset*8 +: CreatorRootKeyShare0Size*8];
            end
            ...
            if (cfg.otp_ctrl_vif.lc_seed_hw_rd_en_i == lc_ctrl_pkg::On) begin
              ...
              exp_creator_root_key.share1 =
                  top_peppermint_rnd_cnst_pkg::RndCnstOtpCtrlPartInvDefault[CreatorRootKeyShare1Offset*8 +: CreatorRootKeyShare1Size*8];
            end
            ...
            // Fetch and verify the CREATOR_SEED seed
            exp_creator_seed.seed_valid = get_otp_digest_val(Secret2Idx) != 0;
            if (cfg.otp_ctrl_vif.lc_seed_hw_rd_en_i == lc_ctrl_pkg::On) begin
              exp_creator_seed.seed =
                  {<<32 {otp_a[CreatorSeedOffset/4 +: CreatorSeedSize/4]}};
            end else begin
              exp_creator_seed.seed =
                  top_peppermint_rnd_cnst_pkg::RndCnstOtpCtrlPartInvDefault[CreatorSeedOffset*8 +: CreatorSeedSize*8];
            end
            `DV_CHECK_EQ(cfg.otp_ctrl_vif.keymgr_creator_seed_o, exp_creator_seed)

            // Fetch and verify the OWNER_SEED seed
            exp_owner_seed.seed_valid = get_otp_digest_val(Secret3Idx) != 0;
            if (cfg.otp_ctrl_vif.lc_seed_hw_rd_en_i == lc_ctrl_pkg::On) begin
              exp_owner_seed.seed =
                  {<<32 {otp_a[OwnerSeedOffset/4 +: OwnerSeedSize/4]}};
            end else begin
              exp_owner_seed.seed =
                  top_peppermint_rnd_cnst_pkg::RndCnstOtpCtrlPartInvDefault[OwnerSeedOffset*8 +: OwnerSeedSize*8];
            end
            `DV_CHECK_EQ(cfg.otp_ctrl_vif.keymgr_owner_seed_o, exp_owner_seed)
  ...
  virtual function void process_core_tl_access(tl_seq_item item, uvm_reg_addr_t csr_addr,
    ...
    case (csr_name)
      ...
      "direct_access_cmd": begin
        if (addr_phase_write && !cfg.otp_ctrl_vif.under_error_states()) begin
          ...
          if (part_idx == VendorTestIdx) begin
            ...
            sw_read_lock = `gmv(ral.rot_firmware_auth_slot1_read_lock) == 0;
          end else if (part_idx == RotFirmwareAuthSlot2Idx) begin
            sw_read_lock = `gmv(ral.rot_firmware_auth_slot2_read_lock) == 0;
          end else if (part_idx == RotFirmwareAuthSlot3Idx) begin
            sw_read_lock = `gmv(ral.rot_firmware_auth_slot3_read_lock) == 0;
          end else if (part_idx == SocFirmwareAuthSlot0Idx) begin
            sw_read_lock = `gmv(ral.soc_firmware_auth_slot0_read_lock) == 0;
          end else if (part_idx == SocFirmwareAuthSlot1Idx) begin
            sw_read_lock = `gmv(ral.soc_firmware_auth_slot1_read_lock) == 0;
          end else if (part_idx == SocFirmwareAuthSlot2Idx) begin
            sw_read_lock = `gmv(ral.soc_firmware_auth_slot2_read_lock) == 0;
          end else if (part_idx == SocFirmwareAuthSlot3Idx) begin
            sw_read_lock = `gmv(ral.soc_firmware_auth_slot3_read_lock) == 0;
      ...
      "rot_firmware_auth_slot3_digest_0", "rot_firmware_auth_slot3_digest_1",
      "soc_firmware_auth_slot0_digest_0", "soc_firmware_auth_slot0_digest_1",
      "soc_firmware_auth_slot1_digest_0", "soc_firmware_auth_slot1_digest_1",
      "soc_firmware_auth_slot2_digest_0", "soc_firmware_auth_slot2_digest_1",
      "soc_firmware_auth_slot3_digest_0", "soc_firmware_auth_slot3_digest_1",
  ...
  virtual function void predict_digest_csrs();
    ...
    void'(ral.rot_firmware_auth_slot0_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[RotFirmwareAuthSlot0Idx]]),
          .kind(UVM_PREDICT_DIRECT)));
    void'(ral.rot_firmware_auth_slot0_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[RotFirmwareAuthSlot0Idx] + 1]),
          .kind(UVM_PREDICT_DIRECT)));

    void'(ral.rot_firmware_auth_slot1_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[RotFirmwareAuthSlot1Idx]]),
          .kind(UVM_PREDICT_DIRECT)));
    void'(ral.rot_firmware_auth_slot1_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[RotFirmwareAuthSlot1Idx] + 1]),
          .kind(UVM_PREDICT_DIRECT)));

    void'(ral.rot_firmware_auth_slot2_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[RotFirmwareAuthSlot2Idx]]),
          .kind(UVM_PREDICT_DIRECT)));
    void'(ral.rot_firmware_auth_slot2_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[RotFirmwareAuthSlot2Idx] + 1]),
          .kind(UVM_PREDICT_DIRECT)));

    void'(ral.rot_firmware_auth_slot3_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[RotFirmwareAuthSlot3Idx]]),
          .kind(UVM_PREDICT_DIRECT)));
    void'(ral.rot_firmware_auth_slot3_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[RotFirmwareAuthSlot3Idx] + 1]),
          .kind(UVM_PREDICT_DIRECT)));

    void'(ral.soc_firmware_auth_slot0_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[SocFirmwareAuthSlot0Idx]]),
          .kind(UVM_PREDICT_DIRECT)));
    void'(ral.soc_firmware_auth_slot0_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[SocFirmwareAuthSlot0Idx] + 1]),
          .kind(UVM_PREDICT_DIRECT)));

    void'(ral.soc_firmware_auth_slot1_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[SocFirmwareAuthSlot1Idx]]),
          .kind(UVM_PREDICT_DIRECT)));
    void'(ral.soc_firmware_auth_slot1_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[SocFirmwareAuthSlot1Idx] + 1]),
          .kind(UVM_PREDICT_DIRECT)));

    void'(ral.soc_firmware_auth_slot2_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[SocFirmwareAuthSlot2Idx]]),
          .kind(UVM_PREDICT_DIRECT)));
    void'(ral.soc_firmware_auth_slot2_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[SocFirmwareAuthSlot2Idx] + 1]),
          .kind(UVM_PREDICT_DIRECT)));

    void'(ral.soc_firmware_auth_slot3_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[SocFirmwareAuthSlot3Idx]]),
          .kind(UVM_PREDICT_DIRECT)));
    void'(ral.soc_firmware_auth_slot3_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[SocFirmwareAuthSlot3Idx] + 1]),
          .kind(UVM_PREDICT_DIRECT)));
  ...
  virtual function bit [TL_DW*2-1:0] get_digest_reg_val(int part_idx);
    ...
    case (part_idx)
      ...
      RotFirmwareAuthSlot1Idx: begin
        digest = {`gmv(ral.rot_firmware_auth_slot1_digest[1]),
                  `gmv(ral.rot_firmware_auth_slot1_digest[0])};
      end
      RotFirmwareAuthSlot2Idx: begin
        digest = {`gmv(ral.rot_firmware_auth_slot2_digest[1]),
                  `gmv(ral.rot_firmware_auth_slot2_digest[0])};
      end
      RotFirmwareAuthSlot3Idx: begin
        digest = {`gmv(ral.rot_firmware_auth_slot3_digest[1]),
                  `gmv(ral.rot_firmware_auth_slot3_digest[0])};
      end
      SocFirmwareAuthSlot0Idx: begin
        digest = {`gmv(ral.soc_firmware_auth_slot0_digest[1]),
                  `gmv(ral.soc_firmware_auth_slot0_digest[0])};
      end
      SocFirmwareAuthSlot1Idx: begin
        digest = {`gmv(ral.soc_firmware_auth_slot1_digest[1]),
                  `gmv(ral.soc_firmware_auth_slot1_digest[0])};
      end
      SocFirmwareAuthSlot2Idx: begin
        digest = {`gmv(ral.soc_firmware_auth_slot2_digest[1]),
                  `gmv(ral.soc_firmware_auth_slot2_digest[0])};
      end
      SocFirmwareAuthSlot3Idx: begin
        digest = {`gmv(ral.soc_firmware_auth_slot3_digest[1]),
                  `gmv(ral.soc_firmware_auth_slot3_digest[0])};
      end
      HwCfg0Idx: begin
    ...
    if (mem_access_allowed &&
      ...
      if (`gmv(ral.rot_firmware_auth_slot0_read_lock) == 0 ||
        ...
        if (partition_start <= addr && addr < partition_end) begin
          ...
          custom_err = 1;
          if (cfg.en_cov) begin
            cov.unbuf_access_lock_cg_wrap[RotFirmwareAuthSlot0Idx].sample(.read_lock(1),
                .write_lock(get_digest_reg_val(RotFirmwareAuthSlot0Idx) != 0), .is_write(0));
          end
          return 0;
        end
      end
      if (`gmv(ral.rot_firmware_auth_slot1_read_lock) == 0 ||
        ...
        if (partition_start <= addr && addr < partition_end) begin
          ...
          custom_err = 1;
          if (cfg.en_cov) begin
            cov.unbuf_access_lock_cg_wrap[RotFirmwareAuthSlot1Idx].sample(.read_lock(1),
                .write_lock(get_digest_reg_val(RotFirmwareAuthSlot1Idx) != 0), .is_write(0));
          end
          return 0;
        end
      end
      if (`gmv(ral.rot_firmware_auth_slot2_read_lock) == 0 ||
        ...
        if (partition_start <= addr && addr < partition_end) begin
          ...
          custom_err = 1;
          if (cfg.en_cov) begin
            cov.unbuf_access_lock_cg_wrap[RotFirmwareAuthSlot2Idx].sample(.read_lock(1),
                .write_lock(get_digest_reg_val(RotFirmwareAuthSlot2Idx) != 0), .is_write(0));
          end
          return 0;
        end
      end
      if (`gmv(ral.rot_firmware_auth_slot3_read_lock) == 0 ||
        ...
        if (partition_start <= addr && addr < partition_end) begin
          ...
          custom_err = 1;
          if (cfg.en_cov) begin
            cov.unbuf_access_lock_cg_wrap[RotFirmwareAuthSlot3Idx].sample(.read_lock(1),
                .write_lock(get_digest_reg_val(RotFirmwareAuthSlot3Idx) != 0), .is_write(0));
          end
          return 0;
        end
      end
      if (`gmv(ral.soc_firmware_auth_slot0_read_lock) == 0 ||
        ...
        if (partition_start <= addr && addr < partition_end) begin
          ...
          custom_err = 1;
          if (cfg.en_cov) begin
            cov.unbuf_access_lock_cg_wrap[SocFirmwareAuthSlot0Idx].sample(.read_lock(1),
                .write_lock(get_digest_reg_val(SocFirmwareAuthSlot0Idx) != 0), .is_write(0));
          end
          return 0;
        end
      end
      if (`gmv(ral.soc_firmware_auth_slot1_read_lock) == 0 ||
        ...
        if (partition_start <= addr && addr < partition_end) begin
          ...
          custom_err = 1;
          if (cfg.en_cov) begin
            cov.unbuf_access_lock_cg_wrap[SocFirmwareAuthSlot1Idx].sample(.read_lock(1),
                .write_lock(get_digest_reg_val(SocFirmwareAuthSlot1Idx) != 0), .is_write(0));
          end
          return 0;
        end
      end
      if (`gmv(ral.soc_firmware_auth_slot2_read_lock) == 0 ||
        ...
        if (partition_start <= addr && addr < partition_end) begin
          ...
          custom_err = 1;
          if (cfg.en_cov) begin
            cov.unbuf_access_lock_cg_wrap[SocFirmwareAuthSlot2Idx].sample(.read_lock(1),
                .write_lock(get_digest_reg_val(SocFirmwareAuthSlot2Idx) != 0), .is_write(0));
          end
          return 0;
        end
      end
      if (`gmv(ral.soc_firmware_auth_slot3_read_lock) == 0 ||
        ...
        if (partition_start <= addr && addr < partition_end) begin
          ...
          custom_err = 1;
          if (cfg.en_cov) begin
            cov.unbuf_access_lock_cg_wrap[SocFirmwareAuthSlot3Idx].sample(.read_lock(1),
                .write_lock(get_digest_reg_val(SocFirmwareAuthSlot3Idx) != 0), .is_write(0));
          end
          return 0;
        end
      end
