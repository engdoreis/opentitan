...
class otp_ctrl_smoke_vseq extends otp_ctrl_base_vseq;
  ...
  constraint dai_wr_legal_addr_c {
    ...
    if (part_idx == AuthSlotStateIdx)
        dai_addr inside `PART_CONTENT_RANGE(AuthSlotStateIdx);
    if (part_idx == RotFirmwareAuthSlot0Idx)
        dai_addr inside `PART_CONTENT_RANGE(RotFirmwareAuthSlot0Idx);
    if (part_idx == RotFirmwareAuthSlot1Idx)
        dai_addr inside `PART_CONTENT_RANGE(RotFirmwareAuthSlot1Idx);
    if (part_idx == RotFirmwareAuthSlot2Idx)
        dai_addr inside `PART_CONTENT_RANGE(RotFirmwareAuthSlot2Idx);
    if (part_idx == RotFirmwareAuthSlot3Idx)
        dai_addr inside `PART_CONTENT_RANGE(RotFirmwareAuthSlot3Idx);
    if (part_idx == SocFirmwareAuthSlot0Idx)
        dai_addr inside `PART_CONTENT_RANGE(SocFirmwareAuthSlot0Idx);
    if (part_idx == SocFirmwareAuthSlot1Idx)
        dai_addr inside `PART_CONTENT_RANGE(SocFirmwareAuthSlot1Idx);
    if (part_idx == SocFirmwareAuthSlot2Idx)
        dai_addr inside `PART_CONTENT_RANGE(SocFirmwareAuthSlot2Idx);
    if (part_idx == SocFirmwareAuthSlot3Idx)
        dai_addr inside `PART_CONTENT_RANGE(SocFirmwareAuthSlot3Idx);
