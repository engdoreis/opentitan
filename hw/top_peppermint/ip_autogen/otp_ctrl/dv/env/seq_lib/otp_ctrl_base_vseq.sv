...
class otp_ctrl_base_vseq extends cip_base_vseq #(
  ...
  virtual task write_sw_digests(bit [NumPartUnbuf-1:0] wr_digest = $urandom());
    ...
    if (wr_digest[RotFirmwareAuthSlot1Idx]) begin
      `DV_CHECK_STD_RANDOMIZE_FATAL(wdata);
      dai_wr(RotFirmwareAuthSlot1DigestOffset, wdata[TL_DW-1:0], wdata[TL_DW*2-1:TL_DW]);
    end
    if (wr_digest[RotFirmwareAuthSlot2Idx]) begin
      `DV_CHECK_STD_RANDOMIZE_FATAL(wdata);
      dai_wr(RotFirmwareAuthSlot2DigestOffset, wdata[TL_DW-1:0], wdata[TL_DW*2-1:TL_DW]);
    end
    if (wr_digest[RotFirmwareAuthSlot3Idx]) begin
      `DV_CHECK_STD_RANDOMIZE_FATAL(wdata);
      dai_wr(RotFirmwareAuthSlot3DigestOffset, wdata[TL_DW-1:0], wdata[TL_DW*2-1:TL_DW]);
    end
    if (wr_digest[SocFirmwareAuthSlot0Idx]) begin
      `DV_CHECK_STD_RANDOMIZE_FATAL(wdata);
      dai_wr(SocFirmwareAuthSlot0DigestOffset, wdata[TL_DW-1:0], wdata[TL_DW*2-1:TL_DW]);
    end
    if (wr_digest[SocFirmwareAuthSlot1Idx]) begin
      `DV_CHECK_STD_RANDOMIZE_FATAL(wdata);
      dai_wr(SocFirmwareAuthSlot1DigestOffset, wdata[TL_DW-1:0], wdata[TL_DW*2-1:TL_DW]);
    end
    if (wr_digest[SocFirmwareAuthSlot2Idx]) begin
      `DV_CHECK_STD_RANDOMIZE_FATAL(wdata);
      dai_wr(SocFirmwareAuthSlot2DigestOffset, wdata[TL_DW-1:0], wdata[TL_DW*2-1:TL_DW]);
    end
    if (wr_digest[SocFirmwareAuthSlot3Idx]) begin
      `DV_CHECK_STD_RANDOMIZE_FATAL(wdata);
      dai_wr(SocFirmwareAuthSlot3DigestOffset, wdata[TL_DW-1:0], wdata[TL_DW*2-1:TL_DW]);
    end
  endtask
  ...
  virtual task write_sw_rd_locks(bit [NumPartUnbuf-1:0] do_rd_lock= $urandom());
    ...
    if (do_rd_lock[RotFirmwareAuthSlot3Idx]) csr_wr(ral.rot_firmware_auth_slot3_read_lock, 0);
    if (do_rd_lock[SocFirmwareAuthSlot0Idx]) csr_wr(ral.soc_firmware_auth_slot0_read_lock, 0);
    if (do_rd_lock[SocFirmwareAuthSlot1Idx]) csr_wr(ral.soc_firmware_auth_slot1_read_lock, 0);
    if (do_rd_lock[SocFirmwareAuthSlot2Idx]) csr_wr(ral.soc_firmware_auth_slot2_read_lock, 0);
    if (do_rd_lock[SocFirmwareAuthSlot3Idx]) csr_wr(ral.soc_firmware_auth_slot3_read_lock, 0);
  ...
  virtual task rd_digests();
    ...
    csr_rd(.ptr(ral.rot_firmware_auth_slot1_digest[1]), .value(val));
    csr_rd(.ptr(ral.rot_firmware_auth_slot2_digest[0]), .value(val));
    csr_rd(.ptr(ral.rot_firmware_auth_slot2_digest[1]), .value(val));
    csr_rd(.ptr(ral.rot_firmware_auth_slot3_digest[0]), .value(val));
    csr_rd(.ptr(ral.rot_firmware_auth_slot3_digest[1]), .value(val));
    csr_rd(.ptr(ral.soc_firmware_auth_slot0_digest[0]), .value(val));
    csr_rd(.ptr(ral.soc_firmware_auth_slot0_digest[1]), .value(val));
    csr_rd(.ptr(ral.soc_firmware_auth_slot1_digest[0]), .value(val));
    csr_rd(.ptr(ral.soc_firmware_auth_slot1_digest[1]), .value(val));
    csr_rd(.ptr(ral.soc_firmware_auth_slot2_digest[0]), .value(val));
    csr_rd(.ptr(ral.soc_firmware_auth_slot2_digest[1]), .value(val));
    csr_rd(.ptr(ral.soc_firmware_auth_slot3_digest[0]), .value(val));
    csr_rd(.ptr(ral.soc_firmware_auth_slot3_digest[1]), .value(val));
  ...
  virtual task force_mubi_part_access();
    ...
    if (cfg.otp_ctrl_vif.alert_reqs == 0 && !cfg.under_reset) begin
      ...
      if ((`gmv(ral.rot_firmware_auth_slot0_digest[0]) ||
          ...
          !$urandom_range(0, 4)) begin
        forced_mubi_part_access[RotFirmwareAuthSlot0Idx].write_lock = 1;
      end
      if ((`gmv(ral.rot_firmware_auth_slot1_digest[0]) ||
           `gmv(ral.rot_firmware_auth_slot1_digest[1])) &&
          !$urandom_range(0, 4)) begin
        forced_mubi_part_access[RotFirmwareAuthSlot1Idx].write_lock = 1;
      end
      if ((`gmv(ral.rot_firmware_auth_slot2_digest[0]) ||
           `gmv(ral.rot_firmware_auth_slot2_digest[1])) &&
          !$urandom_range(0, 4)) begin
        forced_mubi_part_access[RotFirmwareAuthSlot2Idx].write_lock = 1;
      end
      if ((`gmv(ral.rot_firmware_auth_slot3_digest[0]) ||
           `gmv(ral.rot_firmware_auth_slot3_digest[1])) &&
          !$urandom_range(0, 4)) begin
        forced_mubi_part_access[RotFirmwareAuthSlot3Idx].write_lock = 1;
      end
      if ((`gmv(ral.soc_firmware_auth_slot0_digest[0]) ||
           `gmv(ral.soc_firmware_auth_slot0_digest[1])) &&
          !$urandom_range(0, 4)) begin
        forced_mubi_part_access[SocFirmwareAuthSlot0Idx].write_lock = 1;
      end
      if ((`gmv(ral.soc_firmware_auth_slot1_digest[0]) ||
           `gmv(ral.soc_firmware_auth_slot1_digest[1])) &&
          !$urandom_range(0, 4)) begin
        forced_mubi_part_access[SocFirmwareAuthSlot1Idx].write_lock = 1;
      end
      if ((`gmv(ral.soc_firmware_auth_slot2_digest[0]) ||
           `gmv(ral.soc_firmware_auth_slot2_digest[1])) &&
          !$urandom_range(0, 4)) begin
        forced_mubi_part_access[SocFirmwareAuthSlot2Idx].write_lock = 1;
      end
      if ((`gmv(ral.soc_firmware_auth_slot3_digest[0]) ||
           `gmv(ral.soc_firmware_auth_slot3_digest[1])) &&
          !$urandom_range(0, 4)) begin
        forced_mubi_part_access[SocFirmwareAuthSlot3Idx].write_lock = 1;
      end
      ...
      if ((`gmv(ral.rot_firmware_auth_slot1_read_lock) == 0) && !$urandom_range(0, 4)) begin
        forced_mubi_part_access[RotFirmwareAuthSlot1Idx].read_lock = 1;
      end
      if ((`gmv(ral.rot_firmware_auth_slot2_read_lock) == 0) && !$urandom_range(0, 4)) begin
        forced_mubi_part_access[RotFirmwareAuthSlot2Idx].read_lock = 1;
      end
      if ((`gmv(ral.rot_firmware_auth_slot3_read_lock) == 0) && !$urandom_range(0, 4)) begin
        forced_mubi_part_access[RotFirmwareAuthSlot3Idx].read_lock = 1;
      end
      if ((`gmv(ral.soc_firmware_auth_slot0_read_lock) == 0) && !$urandom_range(0, 4)) begin
        forced_mubi_part_access[SocFirmwareAuthSlot0Idx].read_lock = 1;
      end
      if ((`gmv(ral.soc_firmware_auth_slot1_read_lock) == 0) && !$urandom_range(0, 4)) begin
        forced_mubi_part_access[SocFirmwareAuthSlot1Idx].read_lock = 1;
      end
      if ((`gmv(ral.soc_firmware_auth_slot2_read_lock) == 0) && !$urandom_range(0, 4)) begin
        forced_mubi_part_access[SocFirmwareAuthSlot2Idx].read_lock = 1;
      end
      if ((`gmv(ral.soc_firmware_auth_slot3_read_lock) == 0) && !$urandom_range(0, 4)) begin
        forced_mubi_part_access[SocFirmwareAuthSlot3Idx].read_lock = 1;
      end
