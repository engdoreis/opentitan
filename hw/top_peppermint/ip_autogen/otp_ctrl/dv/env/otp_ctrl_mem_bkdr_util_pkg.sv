...
package otp_ctrl_mem_bkdr_util_pkg;
  ...
  function automatic void otp_write_hw_cfg1_partition(
      mem_bkdr_util_pkg::mem_bkdr_util mem_bkdr_util_h,
      bit [SecBootDbgSize*8-1:0] sec_boot_dbg,
    ...
    for (int i = 0; i < SecBootDbgSize; i += 4) begin
      mem_bkdr_util_h.write32(i + SecBootDbgOffset, sec_boot_dbg[i*8+:32]);
      concat_data.push_front(sec_boot_dbg[i*8+:32]);
    end
    word = {
      en_sram_ifetch,
