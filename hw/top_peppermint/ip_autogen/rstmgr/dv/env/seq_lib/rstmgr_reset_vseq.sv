...
class rstmgr_reset_vseq extends rstmgr_base_vseq;
  ...
  task body();
    ...
    // Wait till rst_lc_main_n is inactive for non-aon.
    `DV_WAIT(cfg.rstmgr_vif.resets_o.rst_lc_main_n[1])
    ...
    for (int i = 0; i < num_trans; ++i) begin : trans_loop
      ...
      fork
        ...
        if (which_resets[ResetLowPower]) begin
          cfg.main_clk_rst_vif.wait_clks(lowpower_rst_cycles);
          send_lowpower_reset(.complete_it(0));
        end
        if (which_resets[ResetSw]) begin
          cfg.main_clk_rst_vif.wait_clks(sw_rst_cycles);
          send_sw_reset(.complete_it(0));
        end
        if (which_resets[ResetHw]) begin
          cfg.main_clk_rst_vif.wait_clks(hw_rst_cycles);
      ...
      wait(cfg.rstmgr_vif.resets_o.rst_lc_main_n[1]);
