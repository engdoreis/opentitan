...
class rstmgr_base_vseq extends cip_base_vseq #(
  ...
  localparam int MAIN_FREQ_MHZ = 400;
  ...
  virtual protected task clear_alert_and_cpu_info();
    ...
    send_sw_reset();
    cfg.main_clk_rst_vif.wait_clks(20);  // # of lc reset cycles measured from waveform
  ...
  virtual protected task check_sw_rst_ctrl_n_per_entry(
    ...
    cfg.main_clk_rst_vif.wait_clks(3);
  ...
  local task reset_start(pwrmgr_pkg::reset_cause_e reset_cause);
    ...
    // state transitions.
    cfg.main_clk_rst_vif.wait_clks(rst_to_req_cycles);
  ...
  protected task wait_till_active();
    ...
    cfg.main_clk_rst_vif.wait_clks(8);
  endtask
  ...
  protected task reset_done();
    ...
    cfg.main_clk_rst_vif.wait_clks(10);
    set_reset_cause(pwrmgr_pkg::ResetNone);
    set_pwrmgr_rst_reqs(.rst_lc_req('0), .rst_sys_req('1));
    cfg.main_clk_rst_vif.wait_clks(release_lc_to_release_sys_cycles);
        ...
        foreach (rstreqs[i]) begin : loop
          if (rstreqs[i]) begin
            fork
              ...
              cfg.main_clk_rst_vif.wait_clks(cycles);
  ...
  virtual protected task send_scan_reset(bit complete_it = 1);
    ...
    fork
      begin
        cfg.main_clk_rst_vif.wait_clks(scan_rst_cycles);
        update_scan_rst_n(1'b0);
      end
      begin
        cfg.main_clk_rst_vif.wait_clks(scanmode_cycles);
  ...
  virtual task rstmgr_init();
    // Must set clk_rst_vif frequency to MAIN_FREQ_MHZ since they are gated
    ...
    cfg.clk_rst_vif.set_freq_mhz(MAIN_FREQ_MHZ);
