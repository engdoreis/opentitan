...
"""Split a pickled Peppermint into three deliverable files.
  ...
  <prefix>prim_generic.sv    the technology layer: every design unit that came
                             from a prim_generic FuseSoC core, i.e. the cells an
                             integrator replaces with technology equivalents.
  <prefix>top_packages.sv    the packages needed to instantiate the top
  <prefix>peppermint_rtl.sv  everything else, top module included.

The deliverable names carry no release information; the archive built around
them does.  --release stamps the release identifier into each file, so a file
that has been copied out of that archive can still be traced back.
"""
import argparse
import os
import re
...
def main():
    ...
    ap.add_argument("--release", required=True,
                    help="release identifier stamped into every file, e.g. Peppermint-1.0-M1-RC0")
    args = ap.parse_args()
    ...
    names = {
        "generic": f"{args.prefix}prim_generic.sv",
        "top_packages": f"{args.prefix}top_packages.sv",
        "rtl": f"{args.prefix}peppermint_rtl.sv",
    }
    for i, group in enumerate(order):
        ...
        with open(path, "w") as fh:
            ...
            fh.write(f"// Release {args.release}.\n"
                     ...
                     f"// {args.prefix}*.sv files are one compilation unit and must be read in\n"
