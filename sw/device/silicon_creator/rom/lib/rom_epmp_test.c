...
// The ROM jumps to the nvm_exception_handler first to deal with ECC errors.
// We don't use the NVM exception handler in this test, so we alias the
// symbol to rom_exception_handler to allow the test program to link.
OT_ALIAS("rom_exception_handler")
void nvm_exception_handler(void);
