...
status_t spi_passthrough_demo(context_t *ctx) {
  ...
  do {
      ...
      case kBtnOk:
        switch (selected) {
          case 0:  // Secure boot
            ...
            screen_println(ctx->lcd, "SecureBoot enabled!", alined_center, 5,
                           true);
            break;
          case 1:  // Passthough
            TRY(dif_spi_device_set_passthrough_mode(ctx->spid,
            ...
            TRY(spi_device_testutils_configure_passthrough(
                ...
                /*filters=*/0x00,
                /*upload_write_commands=*/false));
            screen_println(ctx->lcd, "InsecureBoot enabled!", alined_center, 5,
                           true);
            break;
          case 2:
            loop = false;
    ...
    bool flag;
    TRY(dif_spi_device_irq_is_pending(&ctx->spid->dev,
                                      kDifSpiDeviceIrqReadbufWatermark, &flag));
    if (!authenticated && flag) {
      screen_println(ctx->lcd, "Not authenticated!", alined_center, 5, true);
      TRY(dif_spi_device_reset_generic_rx_fifo(ctx->spid));
      TRY(dif_spi_device_irq_acknowledge_all(&ctx->spid->dev));
...
static status_t check_hash(context_t *ctx) {
  ...
  if (memcmp(buf, kHash, ARRAYSIZE(kHash))) {
    ...
    busy_spin_micros(3000 * 1000);
    // TRY_CHECK_ARRAYS_EQ(buf, kHash, ARRAYSIZE(kHash));
