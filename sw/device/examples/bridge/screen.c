...
void screen_init(screen_context_t *screen_ctx) {
  ...
  CHECK_DIF_OK(dif_pinmux_output_select(&pinmux, kTopEarlgreyPinmuxMioOutIoc6, kTopEarlgreyPinmuxOutselSpiHost1Csb));
  CHECK_DIF_OK(dif_pinmux_output_select(&pinmux, kTopEarlgreyPinmuxMioOutIob1, kTopEarlgreyPinmuxOutselSpiHost1Sd0));
  CHECK_DIF_OK(dif_pinmux_output_select(&pinmux, kTopEarlgreyPinmuxMioOutIob2, kTopEarlgreyPinmuxOutselSpiHost1Sck));
  CHECK_DIF_OK(dif_pinmux_output_select(&pinmux, kTopEarlgreyPinmuxMioOutIoc7, kTopEarlgreyPinmuxOutselGpioGpio0));
  CHECK_DIF_OK(dif_pinmux_output_select(&pinmux, kTopEarlgreyPinmuxMioOutIoc9, kTopEarlgreyPinmuxOutselGpioGpio1));
  CHECK_DIF_OK(dif_pinmux_output_select(&pinmux, kTopEarlgreyPinmuxMioOutIob0, kTopEarlgreyPinmuxOutselGpioGpio2));

  dif_pinmux_pad_attr_t out_attr;
  dif_pinmux_pad_attr_t in_attr = {
    .slew_rate = 1,
    .drive_strength = 3,
    .flags = kDifPinmuxPadAttrPullResistorEnable | kDifPinmuxPadAttrPullResistorUp
  };
  ...
  for (int i = 0; i < sizeof(pads) / sizeof(dif_pinmux_index_t); i++) {
    CHECK_DIF_OK(dif_pinmux_pad_write_attrs(&pinmux, pads[i], kDifPinmuxPadKindMio, in_attr, &out_attr));
  }
...
static uint32_t spi_write(void *handle, uint8_t *data, size_t len) {
  ...
  CHECK_DIF_OK(dif_spi_host_transaction(&ctx->spi_host, /*csid=*/0, &transaction, 1));
