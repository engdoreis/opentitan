...
parser.add_argument("--top-level-dir", required=False, help="Path to the top level.")
...
def main(args):
    files = args.file.split(",") if args.file else []
    if args.top_level_dir:
        files += [f for f in glob.glob(f"{args.top_level_dir}/data/*testplan.hjson")]
        files += [f for f in glob.glob(f"{args.top_level_dir}/data/ip/*testplan.hjson")]
...
def export_cmd(args, df: pd.DataFrame):
    ...
    if args.spreadsheet:
        columns = "hw_ip_block name si_stage bazel lc_states tags features otp_mutate boot_stages tests".split()
